// Copyright 2021 The Inet.Af AUTHORS.
...
//
// FromStdIP implicitly unmaps IPv6-mapped IPv4 addresses (such as
// ::ffff:1.2.3.4). That is, if len(std) == 16 and contains
// an IPv4 address, only the IPv4 part is returned, without the
// IPv6 wrapper. This is the common form returned by
...
// netip.AddrFromSlice.
func FromStdIP(std net.IP) (addr netip.Addr, ok bool) {
...
func FromStdAddr(stdIP net.IP, port int, zone string) (_ netip.AddrPort, ok bool) {
	...
	ip = ip.Unmap() // implicitly unmaps IPv6-mapped IPv4 addresses
	if zone != "" {
		if ip.Is4() {
			ok = false
			return
